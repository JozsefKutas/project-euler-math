...
def test_from_quadratic():
    ...
    assert ContinuedFraction.from_quadratic(13, 8) == ContinuedFraction(
        [1, 1, 1, 1, 2], []
    )
    ...
    assert ContinuedFraction.from_quadratic(0, 1, 7) == ContinuedFraction(
        [2], [1, 1, 1, 4]
    )
...
def test_convergents():
    ...
    assert list(ContinuedFraction.from_quadratic(1, 2).convergents()) == [
        (0, 1),
        (1, 2),
    ]
    # Pell's equation
